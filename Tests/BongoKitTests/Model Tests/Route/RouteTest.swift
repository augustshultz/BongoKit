...
class RouteTest: XCTestCase {
    ...
    func testShortNameDecode() throws {
        let json = try Data(dataFromJsonFileNamed: "single_route", forBundle: Bundle(for: type(of: self)))
    ...
    func testDecodingMultipleRoutesFromJSON() throws {
        let json = try Data(dataFromJsonFileNamed: "multiple_routes", forBundle: Bundle(for: type(of: self)))
